...
function Clock() {
  const [timerId, setTimerId] = useState<number | NodeJS.Timer | null>(null);
  ...
  const stringTime = date.toLocaleTimeString("en-GB", { hour12: false }); // часы24:минуты:секунды (01:02:03)/(23:02:03)/(24:00:00)/(00:00:01) // пишут студенты
  const stringDate = `${date.getDate().toString().padStart(2, "0")}.${(date.getMonth() + 1)
    .toString()
    .padStart(2, "0")}.${date.getFullYear()}`; // день.месяц.год (01.02.2022)
